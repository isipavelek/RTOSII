...
#define NEWLINE tiny_putchar('\r');tiny_putchar('\n');
//#define NEWLINE tiny_putchar('\r');

//typedef unsigned char char;
/* redefine some useful and maybe missing utilities to avoid conflicts */
//#define tiny_strlen tinysh_tiny_strlen
//#define tiny_puts tinysh_tiny_puts
#define tiny_putchar tinysh_char_out
...
static void help_fnt(int argc, char **argv)
{
  tiny_puts("?            display help on given or available commands");
  NEWLINE
  tiny_puts("<TAB>        auto-completion");
  NEWLINE
  tiny_puts("<cr>         execute command line");
  NEWLINE
  tiny_puts("CTRL-P       recall previous input line");
  NEWLINE
  tiny_puts("CTRL-N       recall next input line");
  NEWLINE
  tiny_puts("<any>        treat as input character");
}
...
//  tiny_putchar('\n');
  ...
  for (cm = cmd; cm; cm = cm->next)
    if (len < tiny_strlen(cm->name))
      len = tiny_strlen(cm->name);
  for (cm = cmd; cm; cm = cm->next)
    if (cm->help)
    {
      int i;
      tiny_puts(cm->name);
      for (i = tiny_strlen(cm->name); i < len + 2; i++)
        tiny_putchar(' ');
      tiny_puts(cm->help);
//      tiny_putchar('\n');
      NEWLINE
...
//              tiny_putchar('\n');
        ...
        else if (r == PARTMATCH)
        {
          ...
          {
            matched_cmd = cm;
            common_len = tiny_strlen(cm->name);
          }
          else
          {
...
//    int context = 0;
  ...
  else if (c == 16) /* CTRL-P: back in history */
  {
    ...
    if (input_buffers[prevline][0])
    {
      ...
      while (cur_index-- > tiny_strlen(line))
        tiny_puts("\b \b");
//      tiny_putchar('\r');
      NEWLINE
      start_of_line();
      tiny_puts(line);
      cur_index = tiny_strlen(line);
